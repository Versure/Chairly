...
# Commit frontend worktree
cd .worktrees/frontend && git add -A
git diff --cached --quiet || git commit -m "fix({FEATURE_NAME}): address PR review comments (frontend)"
git push origin {FEATURE_BRANCH}

# Merge worktrees back into feature branch in main checkout
git checkout {FEATURE_BRANCH}
git merge --no-ff .worktrees/backend feat/{FEATURE_NAME}/backend \
  -m "chore: merge backend rework fixes" || true
git merge --no-ff .worktrees/frontend feat/{FEATURE_NAME}/frontend \
  -m "chore: merge frontend rework fixes" || true
git push origin {FEATURE_BRANCH}
```
