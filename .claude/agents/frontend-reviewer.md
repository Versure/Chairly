...
description: >
  Frontend code reviewer for Chairly. Checks spec compliance, Sheriff boundaries,
  Angular signals, Dutch UI text, component structure, and e2e coverage.
...
- [ ] Presentational components in `ui/{component-name}/` subfolder (not directly in `ui/`)
...
- [ ] Custom/brand colors have explicit `dark:` variant
...
- [ ] `page.keyboard.press('Escape')` used to close dialogs
