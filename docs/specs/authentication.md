...
- Bounded contexts involved: all (auth is cross-cutting)
- Key entities: `StaffMember` (gains `Email` + `KeycloakUserId` fields)
...
- Per-StaffMember endpoint scoping (e.g., a staff member can only see their own bookings) —
  that's a future data-access-level concern
