...
**Strategy:** instead of hardcoding the path, detect the active worktree by looking for a match in `git worktree list`. The hook already knows the repo root; it can find the correct worktree by scanning `.worktrees/` subdirectories.
